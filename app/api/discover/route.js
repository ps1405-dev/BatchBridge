...
async function chooseChannels(product){try{const ai=new GoogleGenAI({apiKey:process.env.GEMINI_API_KEY});const response=await ai.models.generateContent({model:'gemini-3.6-flash',contents:`Choose up to 3 realistic B2B buyer channels for this product. Return JSON only: {channels:string[]}. Allowed channels: food_retail, beauty_retail, gift_retail, general_retail, hospitality. Product: ${JSON.stringify({name:product.name})}. Do not choose channels that do not make commercial sense.`,config:{responseMimeType:'application/json'}});const channels=JSON.parse(response.text).channels?.filter(x=>CHANNELS[x]);return channels?.length?channels:['general_retail']}catch{return['general_retail']}}
