...
async function chooseChannels(product){try{const ai=new GoogleGenAI({apiKey:process.env.GEMINI_API_KEY});const response=await ai.models.generateContent({model:'gemini-3.6-flash',contents:`Choose up to 3 realistic B2B buyer channels for this product. Return JSON only: {channels:string[]}. Allowed channels: food_retail, apparel_retail, beauty_retail, gift_retail, general_retail, hospitality. Product: ${JSON.stringify({name:product.name})}. Do not choose channels that do not make commercial sense.`,config:{responseMimeType:'application/json'}});const channels=JSON.parse(response.text).channels?.filter(x=>CHANNELS[x]);return channels?.length?channels:['general_retail']}catch{return['general_retail']}}
...
export async function POST(request) {
  try {
    ...
    const seen=new Set();
    const rows=elements.filter(x=>x.tags?.name).map(x=>({maker_id:makerId,product_id:productId,user_id:user.id,business_name:x.tags.name,score:0,rationale:'Awaiting Gemini ranking of this source-linked OpenStreetMap result.',offer:`OpenStreetMap category: ${x.tags.shop||x.tags.amenity||x.tags.tourism||'unknown'}`,outreach_draft:'',latitude:x.lat||x.center?.lat,longitude:x.lon||x.center?.lon,source:'OpenStreetMap',source_url:`https://www.openstreetmap.org/${x.type}/${x.id}`,external_id:`osm:${x.type}:${x.id}`})).filter(row=>!seen.has(row.external_id)&&seen.add(row.external_id)).slice(0,30);
