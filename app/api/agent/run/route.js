...
export async function POST(request) {
  try {
    ...
    const prompt=`You are a wholesale-sales assistant. Rank ONLY these real businesses for the exact selected product below. The product name is authoritative: NEVER append, substitute, or infer another product from the maker's business category. For example, if the selected product is "milk cream", do not call it "milk cream soap". Never invent businesses, ratings, reviews, demand, contact information, or facts beyond supplied data. Use distance as a ranking signal and mention it when available. Make every outreach_draft specific to the exact product name, business name and distance; do not reuse generic wording. Maker context: ${JSON.stringify({name:maker.name,address:maker.address})}. Exact selected product: ${JSON.stringify({name:product.name,wholesale_price:product.wholesale_price,capacity:product.capacity})}. Businesses: ${JSON.stringify(businesses)}. Return JSON only: [{id,score:integer 0-100,rationale,offer,outreach_draft}].`;
