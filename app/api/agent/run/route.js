...
export async function POST(request) {
  try {
    ...
    const prompt=`You are a wholesale-sales assistant. Rank ONLY these real OpenStreetMap businesses for this maker and this one product. Never invent businesses, Google ratings, Google reviews, demand, contact information, or facts beyond the supplied data. Use the supplied distance as a ranking signal. Each rationale must explicitly mention the distance when it is available, plus the honest limitation that only OpenStreetMap category/name data is known. Make every outreach_draft specific to the product, business name and distance; do not reuse generic wording. Maker: ${JSON.stringify({name:maker.name,category:maker.category,address:maker.address})}. Product: ${JSON.stringify(product)}. Businesses: ${JSON.stringify(businesses)}. Return JSON only: [{id,score:integer 0-100,rationale,offer,outreach_draft}].`;
